...
def plot_decision_evolution(cortex_measures_pro, subcortex_measures_pro, cortex_measures_anti, subcortex_measures_anti, total_time, dt, labels, threshold, view=False, save=True):
    ...
    if singleiter:
        ...
        std_cortex = stats.sem(activations_cortex, axis=1)
        ...
        means_cortex = torch.mean(activations_cortex, dim=(0, 2))
        # Manually compute sem since stats.sem require 1d-axis
        std_cortex = torch.std(activations_cortex, dim=(0, 2)) / np.sqrt(activations_cortex.size(0) +
                                                                         activations_cortex.size(2))
        ...
        means_subcortex = torch.mean(activations_subcortex, dim=(0, 2))
        std_subcortex = torch.std(activations_subcortex, dim=(0, 2)) / np.sqrt(activations_subcortex.size(0) +
                                                                               activations_subcortex.size(2))
    ...
    plt.ylabel('Firing rate (a.u.)')
    plt.xlabel('Time (s)')
    plt.title('Mean rates with prosaccade')
    # _, decision = torch.max(activations[-1, :, :], 1)
    # plt.title(f'L:{result[0,0]:.2f}, R:{result[0,1]:.2f}, D:{decision}')
    plt.plot(time, means_cortex[:, 0], label="hit cortex rate", color='red')
    plt.plot([equilibria[0], equilibria[0]], [-0.1, 1.1], '-.', color='red')
    plt.fill_between(time, means_cortex[:, 0]-std_cortex[:, 0], means_cortex[:, 0]+std_cortex[:, 0], alpha=0.3, color='red')
    plt.plot(time, means_cortex[:, 1], label="miss cortex rate", color='orange')
    plt.plot([equilibria[1], equilibria[1]], [-0.1, 1.1], '-.', color='orange')
    plt.fill_between(time, means_cortex[:, 1]-std_cortex[:, 1], means_cortex[:, 1]+std_cortex[:, 1], alpha=0.3, color='orange')
    plt.plot(time, means_subcortex[:, 0], label="hit subcortex rate", color='blue')
    plt.plot([equilibria[2], equilibria[2]], [-0.1, 1.1], '--', color='blue')
    plt.fill_between(time, means_subcortex[:, 0]-std_subcortex[:, 0], means_subcortex[:, 0]+std_subcortex[:, 0], alpha=0.3, color='blue')
    plt.plot(time, means_subcortex[:, 1], label="miss subcortex rate", color='cyan')
    plt.plot([equilibria[3], equilibria[3]], [-0.1, 1.1], '--', color='cyan')
    ...
    if save:
        plt.savefig('Pictures/time/average_prosaccade_time_evolution')
    if view:
        plt.show()
    ...
    if singleiter:
        ...
        left_subcortex = subcortex_measures_anti[:, np.where(labels == 0)[0], :]
        right_subcortex = torch.flip(subcortex_measures_anti[:, np.where(labels == 1)[0], :], dims=[2])
        activations_subcortex = torch.cat((left_subcortex, right_subcortex), dim=1)
        means_subcortex = torch.mean(activations_subcortex, dim=1)
        std_subcortex = stats.sem(activations_subcortex, axis=1)
    else:
        ...
        activations_cortex = torch.cat((left_cortex, right_cortex), dim=2)
        means_cortex = torch.mean(activations_cortex, dim=(0, 2))
        std_cortex = torch.std(activations_cortex, dim=(0, 2)) / np.sqrt(
            activations_cortex.size(0) + activations_cortex.size(2))

        left_subcortex = subcortex_measures_anti[:, :, np.where(labels == 0)[0], :]
        right_subcortex = torch.flip(subcortex_measures_anti[:, :, np.where(labels == 1)[0], :], dims=[3])
        activations_subcortex = torch.cat((left_subcortex, right_subcortex), dim=2)
        means_subcortex = torch.mean(activations_subcortex, dim=(0, 2))
        std_subcortex = torch.std(activations_subcortex, dim=(0, 2)) / np.sqrt(
    ...
    plt.ylabel('Firing rate (a.u.)')
    plt.xlabel('Time (s)')
    plt.title('Mean rates with antisaccade')

    plt.plot(time, means_cortex[:, 1], label="hit cortex rate", color='red')
    plt.plot([equilibria[0], equilibria[0]], [-0.1, 1.1], '--', color='red')
    plt.fill_between(time, means_cortex[:, 1]-std_cortex[:, 1], means_cortex[:, 1]+std_cortex[:, 1], alpha=0.3, color='red')
    plt.plot(time, means_cortex[:, 0], label="miss cortex rate", color='orange')
    plt.plot([equilibria[1], equilibria[1]], [-0.1, 1.1], '--', color='orange')
    plt.fill_between(time, means_cortex[:, 0]-std_cortex[:, 0], means_cortex[:, 0]+std_cortex[:, 0], alpha=0.3, color='orange')
    plt.plot(time, means_subcortex[:, 1], label="hit subcortex rate", color='blue')
    plt.plot([equilibria[2], equilibria[2]], [-0.1, 1.1], '--', color='blue')
    plt.fill_between(time, means_subcortex[:, 1]-std_subcortex[:, 1], means_subcortex[:, 1]+std_subcortex[:, 1], alpha=0.3, color='blue')
    plt.plot(time, means_subcortex[:, 0], label="miss subcortex rate", color='cyan')
    plt.plot([equilibria[3], equilibria[3]], [-0.1, 1.1], '--', color='cyan')
    plt.fill_between(time, means_subcortex[:, 0]-std_subcortex[:, 0], means_subcortex[:, 0]+std_subcortex[:, 0], alpha=0.3, color='cyan')
    ...
    if save:
        plt.savefig('Pictures/time/average_antisaccade_time_evolution')
    if view:
        plt.show()
...
def plot_decision_layer(cortex_measures_pro, subcortex_measures_pro, cortex_measures_anti, subcortex_measures_anti, total_time, dt, labels, view=False, save=True):
    ...
    plt.fill_between(time, means_cortex[:, 0] - std_cortex[:, 0], means_cortex[:, 0] + std_cortex[:, 0], alpha=0.3,
                     color='red')
    plt.plot(time, means_cortex[:, 1], label="R cortex rate", color='orange')
    plt.fill_between(time, means_cortex[:, 1] - std_cortex[:, 1], means_cortex[:, 1] + std_cortex[:, 1], alpha=0.3,
                     color='orange')
    plt.plot(time, means_subcortex[:, 0], label="L subcortex rate", color='blue')
    plt.fill_between(time, means_subcortex[:, 0] - std_subcortex[:, 0], means_subcortex[:, 0] + std_subcortex[:, 0],
                     alpha=0.3, color='blue')
    plt.plot(time, means_subcortex[:, 1], label="R subcortex rate", color='cyan')
    plt.fill_between(time, means_subcortex[:, 1] - std_subcortex[:, 1], means_subcortex[:, 1] + std_subcortex[:, 1],
    ...
    if save:
        plt.savefig('Pictures/time/average_prosaccade_left_time_evolution')
    if view:
        plt.show()
    ...
    if save:
        plt.savefig('Pictures/time/average_prosaccade_right_time_evolution')
    if view:
        plt.show()
    ...
    if save:
        plt.savefig('Pictures/time/average_antisaccade_left_time_evolution')
    if view:
        plt.show()
    ...
    if save:
        plt.savefig('Pictures/time/average_antisaccade_right_time_evolution')
    if view:
        plt.show()
